// scripts/migrate-discipline-grading-index.js
// One-off migration: replace the old unique index { class, week } on
// DisciplineGrading with the correct one { class, schoolYear, week }.
//
...
//
// Run: node scripts/migrate-discipline-grading-index.js
//
// Pre-requisite: scripts/check-discipline-duplicates.js reports 0
// duplicate (class, schoolYear, week) groups. If not, run
// scripts/fix-discipline-duplicates.js first.
...
(async () => {
  try {
    await connectDB();
    const collection = DisciplineGrading.collection;

    // 1. Inspect existing indexes
    const existing = await collection.indexes();
    console.log('=== Current indexes on disciplinegradings ===');
