// scripts/check-class-academic-duplicates.js
...
// Run: node scripts/check-class-academic-duplicates.js
...
(async () => {
  try {
    ...
    const dups = await ClassAcademicGrading.aggregate(pipeline);

    console.log(`\n=== Duplicate (class, schoolYear, week) groups: ${dups.length} ===`);
    if (dups.length > 0) {
      console.log('These groups would BLOCK the new unique index.');
      console.log('Run scripts/fix-class-academic-duplicates.js to resolve them.');
    ...
    const crossYear = await ClassAcademicGrading.aggregate([
      {
        $group: {
