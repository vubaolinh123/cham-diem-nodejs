// scripts/fix-class-academic-duplicates.js
...
//
// Run: node scripts/fix-class-academic-duplicates.js
...
(async () => {
  try {
    ...
    const groups = await ClassAcademicGrading.aggregate([
      {
        $group: {
    ...
    for (const g of groups) {
      ...
      const records = await ClassAcademicGrading.find({ _id: { $in: g.ids } })
        .sort({ createdAt: -1, _id: -1 })
        .select('_id createdAt updatedAt');
      const keep = records[0];
      const remove = records.slice(1).map((r) => r._id);

      const res = await ClassAcademicGrading.deleteMany({ _id: { $in: remove } });
      console.log(
